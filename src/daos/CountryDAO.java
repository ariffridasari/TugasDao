...
//    }
    ...
    public List<Country> getData(Object key, boolean isGetById) {
        ...
        String query = (isGetById)
                ? "SELECT * FROM COUNTRIES WHERE country_id=? ORDER BY country_id"
                : "SELECT * FROM COUNTRIES WHERE country_name LIKE ? OR country_id LIKE ? OR region_id LIKE ? ORDER BY country_id";
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            if (isGetById) {
