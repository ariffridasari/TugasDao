...
public class LocationDAO implements ILocationDAO {
    ...
    public List<Location> getData(Object key, boolean isGetById) {
        ...
        String query = (isGetById)
                ...
                : "SELECT * FROM LOCATIONS WHERE  location_id LIKE (?) OR street_address LIKE (?) OR postal_code LIKE (?)"
        ...
        try {
            ...
            if (isGetById) {
                ...
                preparedStatement.setString(4, "%" + key.toString() + "%");
                preparedStatement.setString(5, "%" + key.toString() + "%");
                preparedStatement.setString(6, "%" + key.toString() + "%");
            }
...
//        List<Location> listLocation = new ArrayList<Location>();
//        String query = "SELECT * FROM LOCATIONS WHERE REGEXP_LIKE(city || street_address ,?,'i')";
////                + " location_id LIKE (?) OR street_address LIKE (?) OR postal_code LIKE (?)"
////                + "OR city LIKE (?) OR state_province LIKE (?) OR country_id LIKE (?)";
//        try {
//            PreparedStatement preparedStatement = connection.prepareStatement(query);
